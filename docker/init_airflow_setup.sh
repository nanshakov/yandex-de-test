...
if [ $IS_INITDB ]; then
  ...
  docker exec -ti airflow airflow db init && echo "Initialized airflow DB"
  docker exec -ti airflow airflow users create --role Admin --username $AIRFLOW_USER --password $AIRFLOW_PASSWORD -e $AIRFLOW_USER_EMAIL -f airflow -l airflow && echo "Created airflow Initial admin user with username $AIRFLOW_USER"
